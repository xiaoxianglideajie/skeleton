        ...
        if i==5%i=5的时候比较特殊，肩膀处会多出来一小块，导致这时候头没法被染成一个颜色,所以i=5的时候单独处理
                for o=1:2
                    ...
                    for e=1:length(headX)
                        mask(headX(e),headY(e))=0;
                    end
                end
        else
            if (abs(findHeadPartAveSortResult(1,1)-findHeadPartAveSortResult(1,2))<66)%认为这时候是头被分开
                for o=1:2
                    [headTempX,headTempY]=find(findHeadPartAveDepth==findHeadPartAveSortResult(1,o));
                    [headX,headY]=find(mask==(findHeadSortTable(headTempY+1,1)));
                    for e=1:length(headX)
...
%    elseif length(findHeadPartAveSortResult)==2
        ...
        if headXave > topLine%要是深度值最大的块算出来的中心在topLine底下，证明这时候这个块是手，则跳过，继续算下一块
...
%% 左右比例标定法
    middleLine=(leftLine+rightLine)/2;
...
%      end 
    ...
    line([middleLine,middleLine],[1,120],'LineWidth',2,'Color',[.8 .8 .2]);
