...
%% 右手画线
    A=[maoRowsOfRightHand,rightLine];
    B=[handOfRightAveX,handOfRightAveY];
    kk=(maoRowsOfRightHand-handOfRightAveX)/(rightLine-handOfRightAveY);
    recordRight=0;%初始化
    recordRight(1,1)=handOfRightAveX;
    recordRight(1,2)=handOfRightAveY;
    recordStart=1;
    xxRight=handOfRightAveX;%赋个初值，避免某时候的时候找不到对应的色块而造成无法画出直线来（报错）
    yyRight=handOfRightAveY;
    
    if handOfRightAveY>rightLine%如果手的中心位置跑到锚点的右侧去，则这时候手的边界是y值最大的点
        for o=1:(Height-floor(handOfRightAveY))
            yyRight=(handOfRightAveY)+o;%递zeng寻找
            xxRight=(kk*yyRight-kk*handOfRightAveY+handOfRightAveX);
            if (round(xxRight)<Width) && (round(xxRight)>1) && (round(yyRight)>1) && (round(yyRight)<Height)
                if mask(round(xxRight),round(yyRight))==500
                    recordRight(recordStart,1)=round(xxRight);%放入堆栈
                    recordRight(recordStart,2)=round(yyRight);
                    recordStart=recordStart+1;%计数加1
                end
            end
        end
        [recordRightRows,recordRightCols]=size(recordRight);
        xxRight=recordRight(recordRightRows,1);%找到边界点
        yyRight=recordRight(recordRightRows,2);
    elseif handOfRightAveY<rightLine%手的中心位置跑到锚点的右侧去
        for o=1:floor(handOfRightAveY)
            yyRight=(handOfRightAveY)-o;%递减寻找
            xxRight=(kk*yyRight-kk*handOfRightAveY+handOfRightAveX);
            
            if (round(xxRight)<Width) && (round(xxRight)>1) && (round(yyRight)>1) && (round(yyRight)<Height) && (mask(round(xxRight),round(yyRight))==500)
                recordRight(recordStart,1)=round(xxRight);%放入堆栈
                recordRight(recordStart,2)=round(yyRight);
                recordStart=recordStart+1;%计数加1
            end
        ...
        if handOfRightAveX<=maoRowsOfRightHand%手的中心在锚点的上面
            for o=1:floor(handOfRightAveX)
                yyRight=(handOfRightAveY);%递减寻找
                xxRight=handOfRightAveX-o;
            ...
            for o=1:floor(Width-handOfRightAveX)
                yyRight=(handOfRightAveY);%递减寻找
                xxRight=handOfRightAveX+o;
...
%     end
     ...
     line([yyRight,handOfRightAveY],[xxRight,handOfRightAveX],'LineWidth',2,'Color',[.8 .8 .2]);%左手的线
